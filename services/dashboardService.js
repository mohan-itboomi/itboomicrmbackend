...
const summary = async date => {
  ...
  const [employees, activeEmployees, projects, onboardProjects, tasks, bugs, employeeDirectory, working, activeSessions, projectTime, employeeTime, activeWork, bugsByProject] = await Promise.all([
    ...
    WorkSession.distinct('employeeId', { status: 'Running' }),
  ...
  return { onboardProjects, counts: { employees, activeEmployees, projects, onboardProjects: onboardProjects.length, tasks, bugs, overdueTasks: overdue, activeTimers: activeSessions.length }, taskStatus, workingHours: working[0] || { minutes: 0, taskMinutes: 0 }, projectTime, employeeTime, employeeDirectory, activeWork, bugsByProject };
};
